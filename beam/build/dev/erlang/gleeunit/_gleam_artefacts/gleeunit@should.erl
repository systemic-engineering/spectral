...
-spec be_ok({ok, IKI} | {error, any()}) -> IKI.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, IKN}) -> IKN.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(IKQ)) -> IKQ.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
