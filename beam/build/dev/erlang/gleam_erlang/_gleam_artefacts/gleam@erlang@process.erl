...
-opaque subject(DLW) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(DLW)}.

-type name(DLX) :: any() | {gleam_phantom, DLX}.

-type do_not_leak() :: any().

-type selector(DLY) :: any() | {gleam_phantom, DLY}.
...
-spec 'receive'(subject(DMW), integer()) -> {ok, DMW} | {error, nil}.
...
-spec receive_forever(subject(DNE)) -> DNE.
...
-spec selector_receive(selector(DNI), integer()) -> {ok, DNI} | {error, nil}.
...
-spec selector_receive_forever(selector(DNM)) -> DNM.
...
-spec map_selector(selector(DNO), fun((DNO) -> DNQ)) -> selector(DNQ).
map_selector(A, B) ->
...
-spec merge_selector(selector(DNS), selector(DNS)) -> selector(DNS).
merge_selector(A, B) ->
...
-spec select_map(selector(DOD), subject(DOF), fun((DOF) -> DOD)) -> selector(DOD).
...
-spec select(selector(DNZ), subject(DNZ)) -> selector(DNZ).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> DON)
) -> selector(DON).
...
-spec select_other(selector(DOR), fun((gleam@dynamic:dynamic_()) -> DOR)) -> selector(DOR).
...
-spec deselect(selector(DOI), subject(any())) -> selector(DOI).
...
-spec select_specific_monitor(selector(DPD), monitor(), fun((down()) -> DPD)) -> selector(DPD).
...
-spec select_monitors(selector(DPG), fun((down()) -> DPG)) -> selector(DPG).
...
-spec select_trapped_exits(selector(DNW), fun((exit_message()) -> DNW)) -> selector(DNW).
...
-spec deselect_specific_monitor(selector(DPJ), monitor()) -> selector(DPJ).
...
-spec send_after(subject(DQF), integer(), DQF) -> timer().
...
-spec perform_call(
    subject(DPM),
    fun((subject(DPO)) -> DPM),
    fun((selector(DPO)) -> {ok, DPO} | {error, nil})
) -> DPO.
...
-spec call(subject(DPT), integer(), fun((subject(DPV)) -> DPT)) -> DPV.
...
-spec call_forever(subject(DPX), fun((subject(DPZ)) -> DPX)) -> DPZ.
