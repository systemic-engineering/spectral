...
-type message(DVH) :: {message, DVH} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(DVI, DVJ) :: {continue,
        DVI,
        gleam@option:option(gleam@erlang@process:selector(DVJ))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(DVK, DVL) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        DVK,
        gleam@erlang@process:selector(message(DVL)),
        gleam@otp@system:debug_state(),
        fun((DVK, DVL) -> next(DVK, DVL))}.

-type started(DVM) :: {started, gleam@erlang@process:pid_(), DVM}.

-opaque initialised(DVN, DVO, DVP) :: {initialised,
        DVN,
        gleam@option:option(gleam@erlang@process:selector(DVO)),
        DVP}.

-opaque builder(DVQ, DVR, DVS) :: {builder,
        fun((gleam@erlang@process:subject(DVR)) -> {ok,
                initialised(DVQ, DVR, DVS)} |
            {error, binary()}),
        integer(),
        fun((DVQ, DVR) -> next(DVQ, DVR)),
        gleam@option:option(gleam@erlang@process:name(DVR))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(DVT) :: {ack, {ok, DVT} | {error, binary()}} |
...
-spec continue(DVY) -> next(DVY, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(DWK, DWL), gleam@erlang@process:selector(DWL)) -> next(DWK, DWL).
...
-spec initialised(DWR) -> initialised(DWR, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(DWW, any(), DWY),
    gleam@erlang@process:selector(DXC)
) -> initialised(DWW, DXC, DWY).
...
-spec returning(initialised(DXH, DXI, any()), DXN) -> initialised(DXH, DXI, DXN).
...
-spec new(DXR) -> builder(DXR, DXS, gleam@erlang@process:subject(DXS)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(DXX)) -> {ok, initialised(DXZ, DXX, DYA)} |
        {error, binary()})
) -> builder(DXZ, DXX, DYA).
...
-spec on_message(builder(DYJ, DYK, DYL), fun((DYJ, DYK) -> next(DYJ, DYK))) -> builder(DYJ, DYK, DYL).
...
-spec named(builder(DYU, DYV, DYW), gleam@erlang@process:name(DYV)) -> builder(DYU, DYV, DYW).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(DZJ))) -> gleam@erlang@process:selector(message(DZJ)).
...
-spec receive_message(self(any(), DZF)) -> message(DZF).
...
-spec initialise_actor(
    builder(any(), any(), EAC),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, EAC} | {error, binary()})
...
-spec start(builder(any(), any(), EAP)) -> {ok, started(EAP)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(EAW), EAW) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(EAY),
    integer(),
    fun((gleam@erlang@process:subject(EBA)) -> EAY)
) -> EBA.
