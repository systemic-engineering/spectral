...
-opaque supervisor(ERA, ERB) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(ERA, ERB))}.

-type message(ERC, ERD) :: any() | {gleam_phantom, ERC, ERD}.

-opaque builder(ERE, ERF) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((ERE) -> {ok, gleam@otp@actor:started(ERF)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(ERE, ERF)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(ERG, ERH) :: {local,
        gleam@erlang@process:name(message(ERG, ERH))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(ERI) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, ERI}.
...
-type erlang_child_spec_property(ERJ, ERK) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((ERJ) -> {ok, gleam@otp@actor:started(ERK)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec get_by_name(gleam@erlang@process:name(message(ERL, ERM))) -> supervisor(ERL, ERM).
get_by_name(Name) ->
...
-spec named(builder(ESC, ESD), gleam@erlang@process:name(message(ESC, ESD))) -> builder(ESC, ESD).
named(Builder, Name) ->
    {builder,
...
-spec restart_tolerance(builder(ESL, ESM), integer(), integer()) -> builder(ESL, ESM).
...
-spec timeout(builder(ESR, ESS), integer()) -> builder(ESR, ESS).
timeout(Builder, Ms) ->
...
-spec restart_strategy(builder(ESX, ESY), gleam@otp@supervision:restart()) -> builder(ESX, ESY).
...
-spec start(builder(ETD, ETE)) -> {ok,
        gleam@otp@actor:started(supervisor(ETD, ETE))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec supervised(builder(EUC, EUD)) -> gleam@otp@supervision:child_specification(supervisor(EUC, EUD)).
supervised(Builder) ->
...
-spec start_child(supervisor(EUJ, EUK), EUJ) -> {ok,
        gleam@otp@actor:started(EUK)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((EVH) -> {ok, gleam@otp@actor:started(EVI)} |
        {error, gleam@otp@actor:start_error()}),
    EVH
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), EVI, gleam@otp@actor:start_error()).
...
-spec worker_child(
    fun((ERS) -> {ok, gleam@otp@actor:started(ERT)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(ERS, ERT).
...
-spec supervisor_child(
    fun((ERX) -> {ok, gleam@otp@actor:started(ERY)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(ERX, ERY).
