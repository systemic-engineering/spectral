...
-type child_specification(EOM) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(EOM)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(EON)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(EON).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(EOS)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(EOS).
supervisor(Start) ->
...
-spec significant(child_specification(EOX), boolean()) -> child_specification(EOX).
...
-spec timeout(child_specification(EPA), integer()) -> child_specification(EPA).
timeout(Child, Ms) ->
...
-spec restart(child_specification(EPD), restart()) -> child_specification(EPD).
...
-spec map_data(child_specification(EPG), fun((EPG) -> EPI)) -> child_specification(EPI).
